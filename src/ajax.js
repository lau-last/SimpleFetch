...
export default class Ajax {
    ...
    // Initializes event delegation and the global map
    constructor() {
        Ajax.delegated = Ajax.delegated || new Map();
    ...
    static ensureListenersFor(triggerAttribute) {
        ...
        const type = triggerAttribute;
        if (Ajax.delegated?.has(type)) return;
        ...
        if (!Ajax.delegated) Ajax.delegated = new Map();
        Ajax.delegated.set(type, handler);
    }


    // Resolves the swap mode (innerHTML by default)
    static resolveAjaxSwap(element) {
        let ajaxSwap = element.getAttribute('data-ajax-swap');
